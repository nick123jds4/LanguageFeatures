...
namespace Users.Infrastructure
{
    ...
    public class DocumentAuthorizationHandler : AuthorizationHandler<DocumentAuthorizationRequirement>
    {
        ...
        {
            var document = context.Resource as ProtectedDocument;
            string user = context.User.Identity.Name;
            var compare = StringComparison.OrdinalIgnoreCase;
            if (document != null && user != null &&
                (requirement.AllowAuthors && document.Author.Equals(user, compare)) ||
                (requirement.AllowEditors && document.Editor.Equals(user, compare)))
            {
